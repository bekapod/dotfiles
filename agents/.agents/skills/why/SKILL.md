...
Before spawning investigators, discover what tools and integrations are available in the current environment. Check for MCP servers, connected services, CLI tools (like `gh`), and any other integrations that can query external systems.

Map each available tool or integration to one evidence category:
...
Source control is always available through git and `gh` (if installed). For the other six, classify using the tool or MCP name, its description, and what it can query. If a tool could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.
...
Launch all matching investigators in parallel if sub-agents are available, otherwise run each sequentially. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple tools.

Investigators need full tool access — they must be able to query MCP servers, run CLI commands, and read files. They should not write or modify anything; that's a posture constraint, not a sandbox setting.
...
- **No tool or integration is available for that category** in this environment. Flag this as a gap, not a choice. Example: "Real-time team chat skipped. No matching tool available, so the conversational record was not searchable."
...
Synthesize the investigator findings. If sub-agents are available, delegate to one; otherwise do it inline. The synthesizer needs full tool access because its quality check spot-verifies citations, which can require querying external tools.
...
- **Collapsing investigators into one agent**. Each tool or integration has its own query vocabulary, result shape, and pitfalls; pooling them dilutes specialization and makes coverage harder to reason about. Always one investigator per category.
