...
This prompt is given to each candidate runner during Phase B along with the task and the Phase A grounding artifacts. Each candidate works independently — in a separate worktree, sub-agent, or sequential pass, depending on what the tool supports. What matters is independence between candidates.
...
You are producing one of several competing candidates. Produce the best design you can; don't hedge. Differences between candidates are the signal used to pick a base and graft the best ideas from others. Converging on a safe-looking middle defeats the exploration.
