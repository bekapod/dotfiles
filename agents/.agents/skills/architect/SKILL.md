...
Generate at least two structurally distinct design candidates. Each candidate produces a design package shaped per `references/rationale-template.md`: the caller's usage written first, then the type sketch, function signatures, module map, and prose rationale derived from it. Use `references/runner-prompt.md` as the brief for each candidate.

When parallel sub-agents are available, spawn one per candidate so they explore independently. Otherwise, produce each candidate sequentially, committing fully to each design before starting the next — don't let the first constrain the second.
...
Synthesize the best elements into one design package. The synthesis decision populates the rationale's "Synthesis decision" section.
...
4. Return to Phase B and generate new candidates.
