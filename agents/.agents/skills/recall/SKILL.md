...
Recent context lives in your conversation history, git log, and any available session transcripts or logs your tool provides.
...
3. Fan out across your history. Use parallel sub-agents if available, otherwise search sequentially. Cover recent conversation history, git log, and any session transcripts or logs available in your environment. Order candidates by real modification time, grep the topic first and then read only matching sessions and only their relevant regions, and skip the current session plus obvious noise (sub-agent, eval, and test sessions). Each search returns the same schema, one block per session: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the session identifier. For one or two sessions, skip the fan-out and search directly. Raw transcripts stay in sub-agents. The main thread gets only their findings.
