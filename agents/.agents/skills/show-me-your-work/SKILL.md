...
At the end of the run, before handing back, check the log told the truth. Review the current session's conversation history (or session transcript if your tool stores one on disk). Walk the log against what actually happened:
